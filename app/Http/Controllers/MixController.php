...
class MixController extends AppBaseController
{
    ...
    public function store(request $request)
    {
        ...
        if($request->size1 !="")
        {
           $recint = DB::table('stocks_details')->where('name','Recin')->first()->total_size;
           $recinTotal =  $recint-$request->size1;
           if($recinTotal <0) {
                Flash::error('Stok Recin Kurang atau sudah habis');
                return redirect(route('mixes.index'));
           }else{
            
            $recints = $recint+$recinTotal;
            $recintUpdate = DB::table('stocks_details')->where('name','Recin')->update(['total_size'=>$recints]); 
        ...
        if($request->size2 !="")
        { 
            $talk = DB::table('stocks_details')->where('name','Talk')->first()->total_size;
            $talkTotal =  $talk-$request->size2;
            if($talkTotal <0) {
                ...
                $talkTotals = $talk+$talkTotal;   
                $talkUpdate = DB::table('stocks')->where('id',$request->kode2)->update(['total_size'=>$talkTotals]); 
        ...
        if($request->size3 !="")
        {
            $katalis = DB::table('stocks_details')->where('name','Katalis')->first()->total_size;
            $katalisTotal =  $katalis-$request->size3;
            if($katalisTotal <0)
            {
               Flash::error('Stok Katalis Kurang atau sudah habis');
                return redirect(route('mixes.index'));
            }else{
                $kataliss = $katalis+$katalisTotal;
                $katalisUpdate = DB::table('stocks')->where('id',$request->kode3)->update(['total_size'=>$kataliss]);
        ...
        if($request->size4 !="")
        {
           $met = DB::table('stocks_details')->where('name','Met')->first()->total_size;
           $metTotal =  $met-$request->size4;
           if($metTotal <0){
        ...
        if($request->size5 !="")
        {
           $dempul = DB::table('stocks_details')->where('name','Dempul')->first()->total_size;
           $dempulTotal =  $dempul-$request->size5;
           if($dempulTotal <0){
    ...
    {    
       ...
       return redirect(route('mixes.index'));
        ...
        Flash::error('Nama Barang Belum Masih Kosong!');
        return redirect(route('mixes.index'));
    }
